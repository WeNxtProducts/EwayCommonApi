		 	...
		 	try {
		 		...
		 		if(StringUtils.isNotBlank(endtTypeId) && !"0".equals(endtTypeId)) {
					 ...
					 if("3".equals(engine.getProductId()) || "19".equals(engine.getProductId())) {
						 ...
						 search="companyId:"+ engine.getInsuranceId() +";productId:"+engine.getProductId()+";riskId:"+riskid+";status:{E,D};requestReferenceNo:"+requestRefercenNo+";";
					 }
	...
	private void loadAndRemoveCoversForEndt(CalcEngine engine, List<Cover> retc, List<Tuple> result) {
		try {
			...
			if(!result.isEmpty()) {
				...
				EndtTypeMaster endtmaster = ratingutil.getEndtMasterData(engine.getInsuranceId(),engine.getProductId(), endtTypeId);
