...
//	List<Map<String,Object>> findByCorporatePlus(String insuranceId,String productId,List<String> loginIds,Date startDate,Date endDate,String usertype);
	...
	@Query(value="SELECT DATE,SUM(QUOTE_COUNT) AS QUOTE_COUNT,SUM(POLICY_COUNT) AS POLICY_COUNT,SUM(QUOTE_PREMIUM) AS QUOTE_PREMIUM, SUM(POLICY_PREMIUM) AS POLICY_PREMIUM,MAX(CURRENCY_CODE) AS CURRENCY_CODE FROM( SELECT (CASE WHEN HPM.STATUS='Y' THEN DATE_FORMAT(HPM.ENTRY_DATE,'%Y-%m-%d') ELSE DATE_FORMAT(HPM.UPDATED_DATE,'%Y-%m-%d') END) AS DATE, (CASE WHEN HPM.STATUS='Y' THEN COUNT(*) END) AS QUOTE_COUNT, (CASE WHEN HPM.STATUS='P' THEN COUNT(*) END) AS POLICY_COUNT, (CASE WHEN HPM.STATUS='Y' THEN ROUND(SUM(OVERALL_PREMIUM_LC),0) END) AS QUOTE_PREMIUM, (CASE WHEN HPM.STATUS='P' THEN ROUND(SUM(OVERALL_PREMIUM_LC),0) END) AS POLICY_PREMIUM, MAX(CPM.CURRENCY_ID) AS CURRENCY_CODE "
			+ "FROM ESERVICE_BUILDING_DETAILS HPM,EWAY_INSURANCE_COMPANY_MASTER CPM WHERE HPM.COMPANY_ID=?1 AND HPM.PRODUCT_ID=?2 AND "
			+ "HPM.STATUS IN ('P','Y') AND (CASE WHEN HPM.STATUS='Y' THEN DATE_FORMAT(HPM.ENTRY_DATE,'%Y-%m-%d') ELSE DATE_FORMAT(HPM.UPDATED_DATE,'%Y-%m-%d') END)"
			+ " BETWEEN DATE_FORMAT(?5,'%Y-%m-%d') AND DATE_FORMAT(?6,'%Y-%m-%d') AND CPM.COMPANY_ID=HPM.COMPANY_ID AND "
			+ "CPM.AMEND_ID=(SELECT MAX(AMEND_ID) FROM EWAY_INSURANCE_COMPANY_MASTER WHERE CPM.COMPANY_ID=COMPANY_ID) AND "
			+ "(CASE WHEN 'Issuer'=?4 THEN HPM.APPLICATION_ID ELSE HPM.LOGIN_ID END) IN (?3) GROUP BY HPM.STATUS,HPM.RENEWAL_DATE_YN,DATE"
			+ " UNION ALL SELECT (CASE WHEN HPM.STATUS='Y' THEN DATE_FORMAT(HPM.ENTRY_DATE,'%Y-%m-%d') ELSE DATE_FORMAT(HPM.UPDATED_DATE,'%Y-%m-%d') END) AS DATE, "
			+ "(CASE WHEN HPM.STATUS='Y' THEN COUNT(*) END) AS QUOTE_COUNT, (CASE WHEN HPM.STATUS='P' THEN COUNT(*) END) AS POLICY_COUNT, (CASE WHEN HPM.STATUS='Y' "
			+ "THEN ROUND(SUM(OVERALL_PREMIUM_LC),0) END) AS QUOTE_PREMIUM, (CASE WHEN HPM.STATUS='P' THEN ROUND(SUM(OVERALL_PREMIUM_LC),0) END) "
			+ "AS POLICY_PREMIUM, MAX(CPM.CURRENCY_ID) AS CURRENCY_CODE FROM ESERVICE_COMMON_DETAILS HPM,EWAY_INSURANCE_COMPANY_MASTER CPM WHERE"
			+ " HPM.COMPANY_ID=?1 AND HPM.PRODUCT_ID=?2 AND HPM.STATUS IN ('P','Y') AND (CASE WHEN HPM.STATUS='Y' THEN DATE_FORMAT(HPM.ENTRY_DATE,'%Y-%m-%d') "
			+ "ELSE DATE_FORMAT(HPM.UPDATED_DATE,'%Y-%m-%d') END) BETWEEN DATE_FORMAT(?5','%Y-%m-%d') AND DATE_FORMAT(?6,'%Y-%m-%d')"
			+ " AND CPM.COMPANY_ID=HPM.COMPANY_ID AND CPM.AMEND_ID=(SELECT MAX(AMEND_ID) FROM EWAY_INSURANCE_COMPANY_MASTER WHERE CPM.COMPANY_ID=COMPANY_ID) "
			+ "AND (CASE WHEN 'Issuer'=?4 THEN HPM.APPLICATION_ID ELSE HPM.LOGIN_ID END) IN (?3) GROUP BY HPM.STATUS,HPM.RENEWAL_DATE_YN,DATE)X GROUP BY DATE",nativeQuery=true)
