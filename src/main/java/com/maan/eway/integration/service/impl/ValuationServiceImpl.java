...
public class ValuationServiceImpl implements ValuationService {
	...
	public PremiaResponse pushValuation(ValuationReq req) {
		...
		if(!CollectionUtils.isEmpty(vlist)) {
			ListItemValue llist=listItemValueRepository.findByItemTypeAndItemCode("VALUATION_INTEGRATION", "2");
			if(llist!=null) {
			...
			for (ValuationIntegration vdata : vlist) {
				...
				try {
					...
					response = restTemplate.postForEntity(llist.getItemValue(), entityReq, ValuationCreateRes.class);
	...
	public String getAccessTokern() {
		String token="";
		try {
		ListItemValue list=listItemValueRepository.findByItemTypeAndItemCode("VALUATION_INTEGRATION", "1");
		if(list!=null) {
			Map<String ,Object> request=new HashMap<String, Object>();
			request.put("email", list.getParam1());
			request.put("password", list.getParam2());
			RestTemplate restTemplate = new RestTemplate();
			HttpHeaders headers = new HttpHeaders();
			headers.setAccept(Arrays.asList(new MediaType[] { MediaType.APPLICATION_JSON }));
			headers.setContentType(MediaType.APPLICATION_JSON);
			HttpEntity<Object> entityReq = new HttpEntity<>(request, headers);
			System.out.println(entityReq.getBody());
			ResponseEntity<ValuationTokenRes> response = restTemplate.postForEntity(list.getItemValue(), entityReq, ValuationTokenRes.class);
			System.out.println(response.getBody());
	...
	public PremiaResponse getStatus(ValuationStatusReq req) {
		PremiaResponse resp=new PremiaResponse();
		try {
			ListItemValue list=listItemValueRepository.findByItemTypeAndItemCode("VALUATION_INTEGRATION", "3");
			if(list!=null) {
				...
				if(!CollectionUtils.isEmpty(vlist)) {
					...
					for (ValuationIntegration vdata : vlist) {
						...
						try {
							...
							HttpEntity<Object> entityReq = new HttpEntity<>(request, headers);
							System.out.println(entityReq.getBody());
							response = restTemplate.exchange(list.getItemValue(),  HttpMethod.POST,entityReq,new ParameterizedTypeReference<List<ValuationStatusDetailsRes>>() {});
							System.out.println(response.getBody());
